...
app = Flask(__name__)
# Render uses the SECRET_KEY variable we set up earlier; fallback to a default locally
app.config['SECRET_KEY'] = os.getenv('SECRET_KEY', 'local-gossip-secret')

# A simple in-memory list to store blogs temporarily (resets on restart)
# Tip: Later, you can connect your DATABASE_URL here!
blog_posts = [
...
<head>
    ...
    <style>
        body { font-family: 'Courier New', monospace; background: #fff0f5; color: #333; padding: 20px; }import os
from flask import Flask, render_template_string, request, redirect, url_for
